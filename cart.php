...
<?php
        ...
        include 'connect.php';
          ...
          //  $password=$_POST["password"];
           ...
           // $hash =  sha1($password);
            $stmt=$con->prepare("select * from cart where username=?");
...
<div id="preloader"></div>
  ...
  <input type="submit" class="example" value="search"style="background-color: rgba(0, 0, 0, 0.6);color: white;" >
    ...
    <header id="fh5co-header" class="fh5co-cover fh5co-cover-sm" role="banner" style="background-image:url(images/bg5.jpg);background-repeat: no-repeat;background-size: 100% 100%; padding: 30%;">
        ...
        <div class="main-w3pvt man-content-wthree text-center" id="home" style="background-color: white">
         ...
         <h1 style="padding-top: 3%; color: #121244">All Uniforms in cart</h1>						
	</div>
 ...
 <?php
                                         foreach ($rows as $rowss) {
                                               $res = mysql_query("SELECT * from products where id='".$rowss['ids']."'");
   ...
   echo '<span class="">'.$row["name"].'</span><br>';
    ...
    echo '<span class = "price">$'.$row["price"].'</span><br>';
     echo " <a class='btn btn-primary' href='pay.php?id=".$rowss["ids"]. "' role='button'>payment</a>";
    echo '</div>';
    echo '</div>';
     echo '</div>';
